...
function Segmented({ value, options, onChange, label }: { value: string; options: string[]; onChange: (value: string) => void; label: string }) {
  return <div className="segmented" role="group" aria-label={label}>{options.map((option) => <button key={option} type="button" className={value === option ? "selected" : ""} aria-pressed={value === option} onClick={() => onChange(option)}>{option}</button>)}</div>;
}
...
export default function Home() {
  ...
  const stepContent: Record<number, React.ReactNode> = {
    ...
    5: <>
      ...
      <div className="panel"><h3>配分方法</h3><Segmented value={state.constraints.distributionMode} options={["equal", "ratio", "random", "shortage"]} label="配分方法" onChange={(value) => patchState({ constraints: { ...state.constraints, distributionMode: value } })} /><div className="segment-labels"><span>均等</span><span>指定比率</span><span>完全ランダム</span><span>不足優先</span></div>{state.constraints.distributionMode === "random" && <Notice tone="warning">完全ランダムでは偏りが生じる可能性があります。学習セットの設計には均等または指定比率を推奨します。</Notice>}</div>
...
function ContractEditor({ title, description, group, items, onAdd, onUpdate, onRemove, usage = false, simple = false }: any) {
  return <section className="panel contract-panel"><div className="panel-header"><div><span className="mini-label">{group.toUpperCase()}</span><h3>{title}</h3><p>{description}</p></div><button type="button" className="button subtle" onClick={onAdd}>＋ 追加</button></div><div className="contract-list">{items.map((item: any) => <div className="contract-row" key={item.id}><input type="checkbox" checked={item.enabled !== false} onChange={(e) => onUpdate(group, item.id, { enabled: e.target.checked })} aria-label={`${item.text || title}を有効にする`} /><input className="grow" value={item.text} placeholder="英語プロンプト" onChange={(e) => onUpdate(group, item.id, { text: e.target.value })} />{usage && <label><span>使用率</span><div><input type="number" min="0" max="100" value={item.usagePercent} onChange={(e) => onUpdate(group, item.id, { usagePercent: Number(e.target.value) })} /><b>%</b></div></label>}{!simple && <label><span>重み</span><input type="number" min="0.5" max="1.5" step="0.05" value={item.promptWeight} onChange={(e) => onUpdate(group, item.id, { promptWeight: Number(e.target.value) })} /></label>}<button type="button" className="icon-button danger" aria-label={`${item.text || "項目"}を削除`} onClick={() => onRemove(group, item.id)}>×</button></div>)}</div></section>;
}
